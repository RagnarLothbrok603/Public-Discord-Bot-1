...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    if (message.author.id != "867497105258315796")
      return message.channel.send({
        embeds: [new MessageEmbed()
          ...
          .setTitle("Only 0x5EC06745#4435 is allowed to deploy the SLASH-COMMANDS")
          .setDescription(`Go to the [Discord-Server](https://discord.gg/GQt8AmASuj), open a Ticket and ask for it!`)
        ]
      });
    try {
      ...
      if(loadSlashsGlobal){
        let themsg = await message.reply(`<a:loading:987385064662913094> **Attempting to set the Global Slash Commands in \`${client.guilds.cache.size} Guilds\`...**`)
        ...
        let guild = client.guilds.cache.get(guildId);
        let themsg = await message.reply(`<a:loading987385064662913094> **Attempting to set the GUILD Slash Commands in \`${guild.name}\`...**`)
