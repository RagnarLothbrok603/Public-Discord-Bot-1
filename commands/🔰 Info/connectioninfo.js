  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    let ls = client.settings.get(message.guild.id, "language")
      ...
      const embed = new Discord.MessageEmbed()
        ...
        .addField('<a:arrow:933154071869796384> **Channel**', `> **${member.voice.channel.name}** ${member.voice.channel}`, true)
        .addField('<a:arrow:933154071869796384> **Channel-ID**', `> \`${member.voice.channel.id}\``, true)
        .addField('<a:arrow:933154071869796384> **Members in there**', `> \`${member.voice.channel.members.size} total Members\``, true)
        .addField('<a:arrow:933154071869796384> **Full Channel?**', `> ${member.voice.channel.full ? "✅" : "❌"}`, true)
        .addField('<a:arrow:933154071869796384> **Bitrate**', `> ${member.voice.channel.bitrate}`, true)
        .addField('<a:arrow:933154071869796384> **User join limit**', `> \`${member.voice.channel.userLimit != 0 ? member.voice.channel.userLimit : "No limit!"}\``, true)
